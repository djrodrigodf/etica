...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.thirdPartyRegistration.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.third-party-registrations.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-4">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('accommodation') ? 'is-invalid' : '' }}" name="accommodation" id="accommodation">
                            ...
                            @foreach(App\Models\ThirdPartyRegistration::ACCOMMODATION_SELECT as $key => $label)
                                <option value="{{ $key }}" {{ old('accommodation', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('food') ? 'is-invalid' : '' }}" name="food" id="food">
                            ...
                            @foreach(App\Models\ThirdPartyRegistration::FOOD_SELECT as $key => $label)
                                <option value="{{ $key }}" {{ old('food', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('food'))
                            <div class="invalid-feedback">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('epi') ? 'is-invalid' : '' }}" name="epi" id="epi">
                            ...
                            @foreach(App\Models\ThirdPartyRegistration::EPI_SELECT as $key => $label)
                                <option value="{{ $key }}" {{ old('epi', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('epi'))
                            <div class="invalid-feedback">
